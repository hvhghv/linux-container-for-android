...
print_modname() {
    ui_print "******************************"
    ui_print "   Linux Container For Android   "
    ui_print "******************************"
    ui_print "     项目作者：hvhghv       "
    ui_print "     本项目仓库链接： https://github.com/hvhghv/linux-container-in-android"
    ui_print "     chroot容器制作方式： https://github.com/hvhghv/chroot-container"
    ui_print "     本脚本基于 'https://github.com/fenildf/magisk-module-installer-translated-chinese-simplified' 模板进行修改"
    ui_print "******************************"
}
...
on_install() {
    ...
    if [ $ARCH == "arm64" ]; then
        tar -xf $MODPATH/system/chroot-ubuntu-2204-01-android-arm64.tar.gz -C $MODPATH/system
    elif [ $ARCH == "x64" ]; then
        tar -xf $MODPATH/system/chroot-ubuntu-2204-01-android-amd64.tar.gz -C $MODPATH/system
    else
        ui_print "- 错误：不支持的架构: $ARCH"
        return 255
    fi

    rm $MODPATH/system/chroot-ubuntu-2204-01-android-arm64.tar.gz
    rm $MODPATH/system/chroot-ubuntu-2204-01-android-amd64.tar.gz
    return 0
}
